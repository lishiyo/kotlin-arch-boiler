...
class CurrencyFragment : Fragment(), MviView<CurrencyIntent, CurrencyViewState> {
  ...
  override fun render(state: CurrencyViewState) {
    ...
    val exchangeResult = state.convertedTotal
    if (quantity != null && exchangeResult != null) {
      val message = quantity.toString() + " " + state.currencyFrom + " = " + exchangeResult.format(4) + " " + state.currencyTo
      showResult(message)
    }
  }
